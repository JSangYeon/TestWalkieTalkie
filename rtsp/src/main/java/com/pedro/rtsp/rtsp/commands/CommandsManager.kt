...
import com.pedro.rtsp.rtsp.RtspClient.Companion.isPause
...
open class CommandsManager {
  ...
  private fun addPlayHeaders(): String {
    return "CSeq: ${++cSeq}\r\n" +
            "User-Agent: ${BuildConfig.LIBRARY_PACKAGE_NAME} ${BuildConfig.VERSION_NAME}\r\n" +
            (if (sessionId == null) "" else "Session: $sessionId\r\n") +
//            "Range: npt=5-20\r\n"
  ...
  fun createPause(): String {
    val pause = "PAUSE rtsp://$host:$port$path RTSP/1.0\r\n" + addHeaders() + "\r\n"
    Log.i(TAG, "${pause}, isPause : ${isPause}")
    isPause = true;
    return pause
  }
  ...
  fun createPlay(): String {
    ...
    val play = "PLAY rtsp://$host:$port$path RTSP/1.0\r\n" + addPlayHeaders() + "\r\n"
